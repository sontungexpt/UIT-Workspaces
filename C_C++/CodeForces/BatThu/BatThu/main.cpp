...
public:
  ...
  friend ostream& operator << (ostream& os, Animal& animal) {
    ...
    os << setw(5) << animal.point << "\t" << setw(4) << animal.percent_rate << endl;
    return os;
  }
...
private:
    ...
    else
    {
      ...
      for (int i = 0; i < length; i++)
        for (int j = i + 1; j < length; j++)
          if (animal_list_copy_sorted[i]->get_point() > animal_list_copy_sorted[j]->get_point())
          {
  ...
  void create_animal_list_rate()
  {
    ...
    else
    {
      ...
      animal_list_copy_sorted[length - 1]->random_percent_rate(max_last_rate, 1);
      int sum_rate = animal_list_copy_sorted[length - 1]->get_percent_rate();
      for (int i = length - 2; i >= 0; i--)
      {
        int relative_rate = ceil(animal_list_copy_sorted[i + 1]->get_point() / animal_list_copy_sorted[i]->get_point());
        int max = animal_list_copy_sorted[i + 1]->get_percent_rate() * relative_rate + 5;
        int min = (animal_list_copy_sorted[i + 1]->get_percent_rate() * relative_rate + animal_list_copy_sorted[i + 1]->get_percent_rate() + 2) / 2;
      ...
      while (sum_rate < 100)
      {
        for (int i = 0; i < length; i++)
        {
          if (sum_rate == 100)
            break;
          animal_list_copy_sorted[i]->set_percent_rate(animal_list_copy_sorted[i]->get_percent_rate() + 1);
          sum_rate++;
        }
      }
      ...
      while (rate_overload > 0)
      {
        for (int i = length - 1; i >= 0; i--)
        {
          ...
          {
            if (animal_list_copy_sorted[i]->get_percent_rate() > animal_list_copy_sorted[i + 1]->get_percent_rate() + 1)
              {
                animal_list_copy_sorted[i]->set_percent_rate(animal_list_copy_sorted[i]->get_percent_rate() - 1);
                rate_overload--;
              }
