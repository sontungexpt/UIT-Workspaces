...
float DemCucTieu(float[][MAXC], int, int);
...
int main()
{
	...
	Xuat(a, m, n);
	cout << "so luong cuc tieu: " << DemCucTieu(a, m, n);
	return 0;
}
//function
...
bool ktCucTieu(float a[][MAXC], int m, int n, int i, int j) // cai nay em k nghi ra thuat toan nen lay tren mang :D mong thay tha thu
{
	if (i == 0 & j == 0)
	{
...
float DemCucTieu(float a[][MAXC], int m, int n)
{
	int dem = 0;
	for (int i = 0; i < m; i++)
		for (int j = 0; j < n; j++)
