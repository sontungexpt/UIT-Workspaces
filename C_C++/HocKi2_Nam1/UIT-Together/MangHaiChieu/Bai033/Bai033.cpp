...
int LeDau(int[][MAXC], int, int);
int LeLonNhat(int[][MAXC], int, int);
...
int main()
{
	...
	Xuat(a, m, n);
	cout << "so le lon nhat la: " << LeLonNhat(a, m, n);
	return 0;
}
//function
...
int LeDau(int a[][MAXC], int m, int n)
{
	for (int i = 0; i < m; i++)
		for (int j = 0; j < n; j++)
			if (a[i][j] % 2 == 0)
...
int LeLonNhat(int a[][MAXC], int m, int n)
{
	int lc = LeDau(a, m, n);
	if (lc == 0)
		return 0;
