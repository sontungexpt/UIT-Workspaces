...
void SapDongTang(float[][MAXC], int, int, int);
void SapDongGiam(float[][MAXC], int, int, int);
...
void SapDongTang(float a[][MAXC], int m, int n, int d)
{
	for (int i = 0; i <= n - 2; i++)
...
void SapDongGiam(float a[][MAXC], int m, int n, int d)
{
	for (int i = 0; i <= n - 2; i++)
		for (int j = i + 1; j <= n - 1; j++)
			HoanVi(a[d][i], a[d][j]);
