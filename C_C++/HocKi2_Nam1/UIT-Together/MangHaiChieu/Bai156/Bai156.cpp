...
void DichXuongCot(float[][MAXC], int, int, int);
void DichXuong(float[][MAXC], int, int);
...
int main()
{
	...
	Xuat(a, m, n);
	cout << "dich xuong xoay vong ma tran: \n";
	DichXuong(a, m, n);
	Xuat(a, m, n);
	return 0;
}
...
void DichXuongCot(float a[][MAXC], int m, int n, int c)
{
	float temp = a[m - 1][c];
	for (int i = m - 1; i >= 1; i--)
		a[i][c] = a[i - 1][c];
...
void DichXuong(float a[][MAXC], int m, int n)
{
	for (int j = 0; j < n; j++)
