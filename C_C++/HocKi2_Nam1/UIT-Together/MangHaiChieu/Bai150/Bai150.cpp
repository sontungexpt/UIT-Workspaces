...
float LonNhatCot(float[][MAXC], int, int, int);
void ThemDong(float[][MAXC], int&, int);
...
float LonNhatCot(float a[][MAXC], int m, int n, int c)
{
	int j = c;
	float lc = a[0][j];
...
void ThemDong(float a[][MAXC], int& m, int n)
{
	m++;
	int i = m - 1;
